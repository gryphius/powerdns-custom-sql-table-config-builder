...
# gmysql-insert-empty-non-terminal-order-query=insert into records (type,domain_id,disabled,name,ordername,auth,change_date,content,ttl,prio) values (null,?,0,?,?,?,NULL,NULL,NULL,NULL)
...
# gmysql-master-zone-query=select master from domains where name=? and type='SLAVE'
...
# gpgsql-list-domain-keys-query=select cryptokeys.id, flags, case when active then 1 else 0 end as active, content from domains, cryptokeys where cryptokeys.domain_id=domains.id and name=$1
...
# gpgsql-master-zone-query=select master from domains where name=$1 and type='SLAVE'
...
# gpgsql-zone-lastchange-query=select max(change_date) from records where domain_id=$1
"""
